...
def highlight_ageing(row):
    if selected_tab == "Open - Complaint - SR":
        ...
        if row["Aging"] < 25:
            ...
            return ["background-color: lightcoral"] * len(row)
    elif selected_tab == "Project Stock at Site":
        if row["Aging"] < 25:
            return ["background-color: lightgreen"] * len(row)
        elif row["Aging"] >= 25:
            return ["background-color: lightcoral"] * len(row)
...
# Define sheet names
sheets_to_count = ["Open - Complaint - SR", "Open Sites", "Stock Liquidation-Project", "Drawing Hold Status", "FG Status", "Reorder"]

# Count rows for each sheet
summary_counts = {sheet: len(df_mapping[sheet]) if sheet in df_mapping else 0 for sheet in sheets_to_count}
...
df_display = filtered_df.iloc[:,3:].copy()
...
edited_df = st.data_editor(
    ...
    column_config={
        "Clearance Date": st.column_config.DateColumn(
            "Clearance Date",
            min_value=datetime.today().date()  # Prevents past dates
        ),
        "Remarks": st.column_config.TextColumn("Remarks"),
        "Support Required": st.column_config.TextColumn("Support Required"),
    },
    disabled=[col for col in df_display.columns if col not in editable_columns],
    hide_index=True,
)
...
def save_to_google_sheet(df, selected_tab):
    try:
        ...
        clear_sheet_mapping = {
            ...
            "Open Sites-Final": "Open Sites-Clear",
